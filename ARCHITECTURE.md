...
Biomni-AD's agent executes LLM-generated Python in-process via `biomni/tool/support_tools.py::run_python_repl`, which calls `exec()`. In a multi-tenant deployment this is the highest-risk surface, so isolation is layered. **Today the image gives you layers 4, 5, 6 and 7; layers 1–3 are deployment-layer configuration the operator must add before multi-tenant exposure.**

1. **[Target] One replica per active user, not one per request.** Configure Container Apps session affinity so a researcher's Chainlit websocket is pinned to one replica (`affinity: sticky`, cookie-based). Combined with per-user volumes (below), this scopes filesystem state to that user. Note this is now a *defence-in-depth* measure rather than a correctness requirement: since item 7, concurrent chats in one process no longer corrupt each other's execution state.
...
6. **[Implemented] Secret scrubbing around the REPL.** `biomni/credentials.py` strips credential-shaped variables from the real `os.environ` for the duration of every `exec()` and restores them after, so LLM-generated code cannot read `ANTHROPIC_API_KEY` and friends - and neither can any subprocess it spawns, which a filtered copy handed to the `exec` globals would not have covered (a bare `import os` defeats that, and children inherit the parent environment regardless). The classifier matches secret-*shaped* names only (`*_API_KEY`, `*_TOKEN`, `*SECRET*`, `*ACCESS_KEY*`, ...), so endpoints and routing config such as `AWS_REGION` and `ENDPOINT_URL` stay readable by legitimate analysis code; `BIOMNI_SCRUB_ENV_EXTRA` / `BIOMNI_SCRUB_ENV_ALLOW` adjust it per deployment. In-process consumers that genuinely need a credential (the LLM factory, the `/readyz` probe) read through `credentials.getenv`, which sees past an open window, so a chat opening during another chat's code step still builds a working client and a probe firing in that window does not report the pod unready. Windows are reference-counted, so overlapping runs share one and no run waits on another.

    **This is not a sandbox.** Any secret this process can recover, code running in this process can recover the same way. It closes the accidental and casual paths - a model that prints its configuration, a library that dumps the environment into a traceback, a snippet that shells out - which is the realistic failure mode. Defending against a determined attacker who knows the codebase needs the executor in a separate process (see the confidential-container / gVisor options below). Deployments open to untrusted users should also scope the key to that deployment and set a provider-side spend cap.

7. **[Implemented] Per-session execution state.** The REPL namespace, captured plots and stdout capture are keyed by chat session (`biomni/tool/support_tools.py`), not process-global as they once were. Previously two people asking a question at the same moment executed against one namespace and one `sys.stdout`: run A received run B's printed output and B's `OUTPUT_DIR`, so A's generated files were written into B's run directory, and a single overlap left the process writing into a discarded buffer, silently swallowing every later print. Stdout is now routed rather than swapped, so no execution can strand the stream. Callers outside a bound session (notebooks, CLI) share one default session, preserving the "variables persist between calls" contract. Session state is capped (`BIOMNI_MAX_REPL_SESSIONS`, default 32) with LRU eviction, since a closed browser tab produces no teardown callback. **Residual:** `matplotlib.pyplot`'s figure manager is process-global, so simultaneous plotting in two sessions can still cross-capture figures.
